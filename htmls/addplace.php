...
<html>
    ...
    <head>        	
            ...
            <title>Új elhelyezési hely.</title>
    </head>
	
    <body>
        <div id="cimsor">
            ÚJ ELHELYEZÉSi HELY HOZZÁADÁSA
        </div>
        <div id="formNewPlace">
	       	<form action='<?php echo $_SERVER['PHP_SELF']; ?>' method='post'>
		        <table class="tableform">
					<tr>
			        	<td>Hely megnevezése:</td><td><input style="border: 2px double red; " class="textbox" type="text" name="hely" size="35"></td>
			        </tr>              
					<tr>
			            <td>Elhelyezés jellege :</td>
			            <td>
			            	<select name="jelleg" class="textbox" onchange="if (this.value=='ujjelleg')   {
								...
								<?php
									...
									$ki="";
									$ki2="";
									$eredmeny=mysql_query("select * from elhelyezes group by jelleg");
        ...
        if (isset($_POST['elkuld'])){
			...
			if ($jelleg=="ujjelleg"){	
				...
				if ($jelleg!=""){	
					ConDb();
                	$ki='';
                	$sql=mysql_query("select jelleg from elhelyezes where azon='$jelleg'");
			...
			if(($hely=="")||($jelleg=="")){
                ...
                ConDb();
                $ki='';
                $sql=mysql_query("select hely from elhelyezes where hely='$hely'");
                while($adat=mysql_fetch_array($sql)){
                	$ki=$adat;
				}
				ClsConDb();
				if ($ki!=''){
                	...
                	ConDb();
                    $sql="INSERT INTO elhelyezes VALUES (null,'$hely','$jelleg')";
